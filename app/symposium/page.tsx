...
import { formatBonusValue } from "@/lib/webinar-access"
      ...
      <main className="pt-24">
        <section className="py-20 px-4 bg-gradient-to-br from-primary/5 to-secondary/5">
          <div className="max-w-6xl mx-auto">
            ...
            <p className="text-md text-muted-foreground mb-4">Date: {symposiumSchedule.date}</p>

            <div className="inline-flex items-center gap-2 bg-amber-50 border border-amber-200 rounded-lg px-4 py-2 mt-2">
              <Gift className="h-5 w-5 text-amber-600" />
              <span className="text-amber-800 text-sm font-medium">
                Includes complimentary access to 4 exclusive webinars (value: {formatBonusValue()})
              </span>
            </div>
          </div>
        </section>

        <section className="py-8 px-4 bg-gradient-to-r from-amber-50 to-orange-50 border-y border-amber-100">
          <div className="max-w-6xl mx-auto">
            <div className="flex flex-col md:flex-row items-start md:items-center gap-4 md:gap-8">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-semibold text-amber-900">Bonus Webinar Access</h3>
                  <p className="text-sm text-amber-700">
                    All symposium registrants receive complimentary access to four pre-conference webinars, providing
                    foundational knowledge and extended learning opportunities.
                  </p>
                </div>
              </div>
            </div>
              ...
              {symposiumSchedule.agenda.map((item, index) => (
                <div
                  ...
                  className={`grid grid-cols-1 md:grid-cols-12 gap-4 p-4 rounded-lg border ${
                    item.activity.includes("DISCUSSION") ||
                    item.activity.includes("Break") ||
                    item.activity.includes("Industrial Symposium")
