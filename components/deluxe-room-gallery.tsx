  ...
  return (
    <>
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        ...
        >
          <VisuallyHidden>
            <DialogTitle>Deluxe Room Gallery - {deluxeRoomImages[currentIndex].title}</DialogTitle>
          </VisuallyHidden>
          ...
          <div className="absolute top-4 left-4 z-50 text-white/90 text-sm font-medium bg-black/50 px-3 py-1 rounded-full">
            {currentIndex + 1} / {deluxeRoomImages.length}
          </div>
          ...
          <div className="relative w-full h-full flex items-center justify-center">
            <div className="relative w-full h-[70vh] max-h-[600px]">
              <Image
                src={deluxeRoomImages[currentIndex].src || "/placeholder.svg"}
                alt={deluxeRoomImages[currentIndex].alt}
                fill
          ...
          <div className="absolute bottom-20 left-0 right-0 text-center">
            <p className="text-white font-medium text-lg">{deluxeRoomImages[currentIndex].title}</p>
          </div>

          {/* Thumbnail Strip */}
          <div className="absolute bottom-4 left-0 right-0 flex justify-center gap-2 px-4 overflow-x-auto">
            {deluxeRoomImages.map((image, index) => (
              <button
                key={index}
